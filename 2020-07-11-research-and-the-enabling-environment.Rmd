...
Was there an impending global famine in the 1960s? Was population on a trajectory to outpace food production?

There were certainly many loud "experts" proclaiming as much from the end of World War II up through the 1970s [@paddock1967famine; @ehrlich1971population; @osborn1949our; @vogt1948road]. Sensational speculations such as the following were typical:

>"[T]he battle to feed all of humanity is over... In the 1970s and 1980s hundreds of millions of people will starve to death in spite of any crash programs embarked upon now" [@ehrlich1967paying].
...
As far back as 1951, President Truman viewed food aid as a "diplomatic weapon" against the spread of "Communism" [@mcmahon1987food].
...
this_file <- "USDA Beginning Stocks 1960-2021.csv"
this_filepath <- paste0(this_folder, this_file)
...
df_stocks <- df_stocks %>% gather(Year, `Beginning Stocks`, `1960/1961`:`2020/2021`)
df_stocks$Year <- as.integer(gsub(".*/", "", df_stocks$Year))
df_stocks$`Beginning Stocks` <- as.numeric(gsub(",", "", df_stocks$`Beginning Stocks`))
# Convert to units of million mt
df_stocks$`Beginning Stocks` <- df_stocks$`Beginning Stocks` / 1000
colnames(df_stocks)[ncol(df_stocks)] <- "Beginning Stocks (million metric tons)"
...
df_stocks <- df_stocks %>% group_by(Country, Commodity, Year) %>%
  summarise(`Beginning Stocks (million metric tons)` = sum(`Beginning Stocks (million metric tons)`, na.rm = T)) %>%
  as.data.frame()
...
df_plot <- df_plot %>% group_by(Country, Commodity) %>%
  mutate(mu_stocks = mean(`Beginning Stocks (million metric tons)`)) %>%
  as.data.frame()
...
gg <- ggplot(df_plot, aes(x = Year,
                          y = `Beginning Stocks (million metric tons)`,
                          fill = Commodity))
gg <- gg + geom_area(position = "stack")
gg <- gg + scale_fill_manual(values = color_vec)
