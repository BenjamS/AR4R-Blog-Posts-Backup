---
title: Intro to MV analysis
author: Ben Schiek
date: '2020-07-19'
slug: intro-to-mv-analysis
categories:
  - Methodology
tags: []
bibliography: Intro to MV analysis.bib
---
...
df_ohlcv <- read.csv(this_filepath, stringsAsFactors = F, header = F)
df_ohlcv$X <- NULL
colnames(df_ohlcv) <- df_ohlcv[1, ]
df_ohlcv <- df_ohlcv[-1, ]
...
```{r, fig.show = 'hold', fig.cap="\\label{fig:hReturns}Historical (top) daily returns and (bottom) period return, financial data.", fig.width=6, fig.height=6, fig.align="center", echo=FALSE}
#=======================================================================
...
```{r mvConv, fig.show = "hold", fig.width = 4, fig.height=4, fig.align="center", fig.cap="Optimal portfolio returns frontier and budget shares, financial data.", echo = FALSE}
#=======================================================================
...
Finally, note that the real ROI to the optimized portfolios over the investment period turn out to be much lower than that indicated by the efficient frontier (Figure \@ref(fig:mvROILevgBtest), right panel). This is a common problem encountered in MV Analysis. Optimal solutions are highly sensitive to noise in the covariance matrix, often resulting in efficient frontiers that overstate returns and/or understate risk [@michaud1989markowitz].
...
...Figure \@ref(fig:hReturns)

```{r hReturns, fig.show = 'hold', fig.cap="Historical (top) daily returns and (bottom) period return, financial data.", fig.width=6, fig.height=6, fig.align="center", echo=FALSE}
# Plot historical returns for period and items defined above
fig_title <- "Period Returns (%)"
list_graph_options <- list()
list_graph_options[["fig_title"]] <- fig_title
